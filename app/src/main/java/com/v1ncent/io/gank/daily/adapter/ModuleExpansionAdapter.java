...
public class ModuleExpansionAdapter extends RecyclerView.Adapter<ModuleExpansionAdapter.ModuleViewHolder> {
    private Context context;
    private List<DayDateResult.ResultsBean.拓展资源Bean> detailsList;
    private int logoid[] = {
    ...
    public ModuleExpansionAdapter(Context context, List<DayDateResult.ResultsBean.拓展资源Bean> detailsList) {
        this.context = context;
        this.detailsList = detailsList;
    }

    @Override
    public ModuleExpansionAdapter.ModuleViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_module, parent, false);
        return new ModuleExpansionAdapter.ModuleViewHolder(view);
    }

    @Override
    public void onBindViewHolder(ModuleExpansionAdapter.ModuleViewHolder holder, int position) {
        DayDateResult.ResultsBean.拓展资源Bean details = detailsList.get(position);
        ...
        if (null != details.getImages()
                && details.getImages().size() > 0) {
            Glide
                    .with(context)
                    .load(details.getImages().get(0))
                    .asBitmap()
                    .placeholder(R.color.white)
                    .error(R.mipmap.img_load_error)
                    .into(holder.item_module_img);
        } else {
            holder.item_module_img.setVisibility(View.GONE);
        }
